...
var report = {
    ...
    'transforms': {'status': 'ok', "comments": `<p>Hungarian has a number of digraphs, like 'gy', 'ty', 'sz', that refer to distinct, different sounds. However, (strangely) the typography rules do not follow this: capitalization is "Gyomor", instead of "GYomor". Ie, they are not considered, typographically, as single letters (in contrast to the Dutch "ij"). Consequently the usual transformations work.</p>` },
    ...
    'hyphenation': { 'status': 'broken', "comments": `<p>Few browsers so far support hyphenation. It works on FF (yay!) but it does not work on Chrome or Safari.</p><p>Hyphenation has some more complex rules (handling compound words, digraphs, rules on repeated letters expressing emphasis-like longer sounds, etc.). It would need a more systematic test to decide whether everything is fine on FF but, at first glance, it seems to be working o.k. It does not use the break opportunities when digraphs are duplicated. E.g., in "Összeg" the "ssz" is a shortened form for "szsz", i.e., a double "sz" digraph; when hyphenating, one could do a "Ösz-szeg" which FF doesn't do. That being said, it is a bit of an edge case and people often do what FF does. I.e., I guess an "ok" status is fine <em>for FF</em>. But the non-existent hyphenation in other browsers means that the feature is still broken.</p>` },
    'justification': { 'status': 'ok', "comments": `<p></p>` },
    'counters': { 'status': 'ok', "comments": `<p></p>` },
    'initialletter': { 'status': 'ok', "comments": `<p>Just as for capitalization, Hungarian would use, e.g., dropcaps for the first character only even in the case of digraphs (I checked it some more typographically demanding books). Ie, the usual behaviour is fine.</p>` },
...
<li>Although I presume this is more a matter for Unicode, but the sorting order regarding diagraphs is that those are considered to be different letters; in the order of letters, the diggraph immediately follows the first letter. I.e., "c" would be followed by "cs" and before the "d" in the ABC, and the word “cukor” (sugar) would precede the word “csupor” (mug) in a dictionary, because "cs" is a diagraph.
</ul>` },
